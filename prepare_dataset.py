...
class CustomDatasetClassify(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {"input_ids": input_ids, "attention_mask":attention_mask, "attr_labels":attr_labels}
...
def create_dataset(raw_datasets, data_args, training_args, tokenizer, mode='train'):
    if mode=='train':
        ...
        with training_args.main_process_first(desc="train dataset map pre-processing"):
            ...
            train_dataset_classify = preprocess_function_classify(train_dataset, tokenizer, data_args)
        return train_dataset_generate, train_dataset_classify
        ...
        with training_args.main_process_first(desc="validation dataset map pre-processing"):
            eval_dataset_generate = preprocess_function_generate(eval_dataset, tokenizer, data_args)
            eval_dataset_classify = preprocess_function_classify(eval_dataset, tokenizer, data_args)
        return eval_dataset_generate, eval_dataset_classify
    elif mode=='test':
        max_target_length = data_args.val_max_target_length
        ...
        with training_args.main_process_first(desc="prediction dataset map pre-processing"):
            predict_dataset_generate = preprocess_function_generate(predict_dataset, tokenizer, data_args)
            predict_dataset_classify = preprocess_function_classify(predict_dataset, tokenizer, data_args)
        return predict_dataset_generate, predict_dataset_classify
